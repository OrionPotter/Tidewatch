...
-- 删除已存在的表（如果需要重新初始化）
-- DROP TABLE IF EXISTS kline_update_log;
-- DROP TABLE IF EXISTS monitor_data_cache;
-- DROP TABLE IF EXISTS stock_kline_data;
-- DROP TABLE IF EXISTS monitor_stocks;
...
-- monitor_stocks表触发器
CREATE TRIGGER IF NOT EXISTS update_monitor_stocks_updated_at
    AFTER UPDATE ON monitor_stocks
    FOR EACH ROW
BEGIN
    UPDATE monitor_stocks SET updated_at = CURRENT_TIMESTAMP WHERE id = NEW.id;
END;

-- stock_kline_data表触发器
CREATE TRIGGER IF NOT EXISTS update_stock_kline_data_updated_at
    AFTER UPDATE ON stock_kline_data
    FOR EACH ROW
BEGIN
...
-- 插入初始化数据
-- 投资组合初始数据
INSERT OR IGNORE INTO portfolio (code, name, cost_price, shares) VALUES
...
-- 监控股票初始数据
INSERT OR IGNORE INTO monitor_stocks (code, name, timeframe, reasonable_pe_min, reasonable_pe_max) VALUES
...
-- 监控股票汇总视图
CREATE VIEW IF NOT EXISTS monitor_stocks_summary AS
SELECT 
    COUNT(*) as total_stocks,
...
-- 最近K线更新状态视图
CREATE VIEW IF NOT EXISTS latest_kline_update AS
SELECT 
    update_date,
    success_count,
    total_count,
